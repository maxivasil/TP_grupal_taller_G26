...
void HUD::render(SDL2pp::Renderer& renderer, const HUDData& data) {
    ...
    try {
        ...
        SDL2pp::Surface speedSurface =
                font->RenderText_Solid(speedText.str(), SDL_Color{0, 255, 0, 255});
        SDL2pp::Surface healthSurface =
                font->RenderText_Solid(healthText.str(), SDL_Color{255, 0, 0, 255});
        SDL2pp::Surface checkpointSurface =
                font->RenderText_Solid(checkpointText.str(), SDL_Color{0, 255, 255, 255});
        SDL2pp::Surface timeSurface =
                font->RenderText_Solid(timeText.str(), SDL_Color{255, 255, 255, 255});
        ...
        int padding = int(10 * hudScale);
        int lineHeight = int(25 * hudScale);
        int x = padding;
        int y = padding;
        ...
        // Render checkpoint
        renderer.Copy(
                checkpointTexture, SDL2pp::NullOpt,
        ...
        // ========== RENDER MUTE BUTTON (BOTTOM-RIGHT CORNER WITH TEXT) ==========
        ...
        // Button background color based on audio state
        if (audioState == AudioState::MUTED) {
            renderer.SetDrawColor(200, 50, 50, 200);  // Red: muted
        } else if (audioState == AudioState::MUSIC_ONLY) {
            renderer.SetDrawColor(100, 150, 200, 200);  // Blue: effects only (no music)
        } else {
            renderer.SetDrawColor(50, 150, 50, 200);  // Green: full sound
        }
        ...
        // Button text based on audio state
        std::string buttonText;
        if (audioState == AudioState::MUTED) {
            buttonText = "MUTED";  // Complete silence
        } else if (audioState == AudioState::MUSIC_ONLY) {
            buttonText = "EFFECTS";  // Effects only (no music)
        } else {
            buttonText = "SOUND";  // Full sound
        }
...
void HUD::onWindowResize(int w, int h, float scale) {
    windowWidth = w;
    windowHeight = h;
    hudScale = scale;
