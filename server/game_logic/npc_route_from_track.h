...
public:
    ...
    static std::vector<NPCRoute> generateRoutesFromTrackCheckpoints(
            const std::vector<RoutePoint>& trackCheckpoints, int numRoutes = 5) {
        ...
        for (int routeIdx = 0; routeIdx < numRoutes; ++routeIdx) {
            std::vector<RoutePoint> route =
                    generateVariationOfTrack(trackCheckpoints, routeIdx, numRoutes);
    ...
    static std::vector<RoutePoint> generateVariationOfTrack(
            const std::vector<RoutePoint>& originalTrack, int routeIndex, int totalRoutes) {

        std::vector<RoutePoint> variation;

        if (originalTrack.size() < 2)
            return variation;
        ...
        for (size_t i = 0; i < originalTrack.size(); ++i) {
            ...
            variation.push_back(RoutePoint(current.x + offsetX + direction.x * longitudinalVar,
